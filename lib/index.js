...
AC.prototype.check=function(opt){
	...
	return function checkit(req,res,next){
		...
		var clientReq=getClientRequest(req);
		...
		if(!token){
			...
			var options={url:self.options.server.userAuthURL,json:{token:token,resource:scope,operation:operation,clientRequest:clientReq},method:'POST'};
