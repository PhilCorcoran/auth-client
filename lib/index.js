...
AC.prototype.angXSRF = function(noRespond) {
    ...
    return function xsrf(req, res, next) {
        if(validApiKey(req,self.options.apiKey)) return next();
...
AC.prototype.check = function(opt) {
    ...
    return function checkit(req, res, next) {
        ...
        var token = req[self.options.tokenName] || req.body[self.options.tokenName] || req.query[self.options.tokenName] || req.cookies[self.options.tokenName] || getAuthHeader(req);
    ...
    function redirectOr401(req, res, message) {
        ...
        if(req.cookies[self.options.tokenName])
...
function getAuthHeader(req){
    ...
    debug('getAuthHeader() Bearer:'+bearer); 
    return bearer; 
}
...
function validApiKey(req,apiKey){
    ...
    if(!headerApiKey){
        debug('X-Api-Key not sent in request');
        return false;
    }
    if(headerApiKey.trim()==apiKey.trim()){
        debug('X-Api-Key is good');
        return true;
    }
